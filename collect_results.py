...
for desc in ("muon", "adam"):
    rhos = sorted({r["rho"] for r in rows.values()
                   if r["descent"] == desc and r["rho"] > 0
                   and r["scale"] == "relative"})
    if not rhos:
        continue
    b = anchor.get(desc)
    print(f"=== descent = {desc}   (rho=0 baseline {b:.4f})" if b else f"=== descent = {desc}")
    print(f"{'ascent':<10}" + "".join(f"{('rho='+str(r)):>12}" for r in rhos))
    for asc in ("mome", "adam", "muon"):
        cells = []
        for rho in rhos:
            hit = [r for r in rows.values() if r["ascent"] == asc
                   and r["descent"] == desc and r["rho"] == rho
                   and r["scale"] == "relative"]
            if not hit:
                cells.append(f"{'-':>12}")
            elif not hit[0]["complete"]:
                cells.append(f"{'(running)':>12}")
            else:
                d = hit[0]["loss"] - b if b else 0
                cells.append(f"{hit[0]['loss']:.4f}{d:+.4f}".rjust(12))
        if any("-" != c.strip() for c in cells):
            print(f"{asc:<10}" + "".join(cells))
    print()
